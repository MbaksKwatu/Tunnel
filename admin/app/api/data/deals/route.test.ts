...
const rows = [{ id: 'd1', name: 'Deal One', company_name: 'Acme', currency: 'KES', created_at: '2026-07-01T00:00:00Z' }]
...
describe('GET /api/data/deals', () => {
  ...
  it('returns pds_deals data when authenticated', async () => {
    const { GET } = await import('./route')
    const res = await GET()
