...
export default function DealDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setDocuments((docsRes.data as PdsDocument[]) ?? [])
  ...
  if (loading) {
    return (
      <div style={{ padding: '40px', color: 'var(--t2)' }}>Loading…</div>
    )
  }

  if (!deal) {
    return (
      <div style={{ padding: '40px', color: 'var(--red)' }}>Deal not found.</div>
    )
  }
    ...
    <div style={{ padding: '40px' }}>
      <button
        ...
        style={{
          ...
          color: 'var(--teal)', fontFamily: "'IBM Plex Mono', monospace",
      ...
      <div style={{
        ...
        {[
          ...
          <div key={item.label} style={{
            ...
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, letterSpacing: '0.08em', color: 'var(--t2)', marginBottom: 6 }}>
              {item.label.toUpperCase()}
            </div>
            <div style={{
              fontFamily: item.mono ? "'IBM Plex Mono', monospace" : "'IBM Plex Sans', sans-serif",
              fontSize: item.mono ? 11 : 14, color: 'var(--t0)',
            }}>
              {item.value ?? '—'}
            </div>
          </div>
      ...
      <section style={{ marginBottom: 40 }}>
        <h2 style={{
          fontFamily: "'IBM Plex Serif', serif", fontWeight: 400, fontSize: 18,
